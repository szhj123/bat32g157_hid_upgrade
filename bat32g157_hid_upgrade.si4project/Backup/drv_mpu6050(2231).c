...
//返回值:0,设置成功
//    其他,设置失败
uint8_t Drv_MPU_Set_Gyro_Fsr(uint8_t fsr)
{
	return Drv_MPU_Write_Byte(MPU_ADDR, MPU_GYRO_CFG_REG, (fsr << 3) | 3); //设置陀螺仪满量程范围
}
...
//返回值:0,设置成功
//    其他,设置失败
uint8_t Drv_MPU_Set_Accel_Fsr(uint8_t fsr)
{
	return Drv_MPU_Write_Byte(MPU_ADDR, MPU_ACCEL_CFG_REG, fsr << 3); //设置加速度传感器满量程范围
}
...
uint8_t MPU_Set_LPF(uint16_t lpf)
{
	...
	return Drv_MPU_Write_Byte(MPU_ADDR, MPU_CFG_REG, data); //设置数字低通滤波器
}
...
//返回值:0,设置成功
//    其他,设置失败
uint8_t Drv_MPU_Set_Rate(u16 rate)
{
	uint8_t data;
	if (rate > 1000)
		rate = 1000;
	...
	data = Drv_MPU_Write_Byte(MPU_ADDR, MPU_SAMPLE_RATE_REG, data); //设置数字低通滤波器
...
void Drv_MPU_Delay_Ms(uint8_t nms )
{
    mpu_delay_cnt = 0;
